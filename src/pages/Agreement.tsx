...
const Agreement: React.FC = () => {
  ...
  const checkVerifiedAndContinue = async () => {
    ...
    if (freshUser && freshUser.emailVerified) {
      ...
      setToast('Email still not verified. Check your inbox or resend the verification email.')
    }
    setLoading(false)
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-white px-6 py-12">
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-lg p-8">
        ...
        <div className="prose max-w-none mb-4">
          ...
          <p>By using Accredited Financial Services you agree to our terms...</p>
        </div>

        <div className="mb-4">
          <label className="block text-sm mb-2">Type your full name to sign</label>
          <input className="w-full border rounded px-3 py-2" value={signName} onChange={e => setSignName(e.target.value)} />
          <label className="inline-flex items-center mt-3"><input type="checkbox" className="mr-2" checked={signed} onChange={e => setSigned(e.target.checked)} /> I agree and sign</label>
        </div>
        ...
        {user && !user.emailVerified ? (
          <div className="space-y-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button className="bg-blue-700 text-white px-4 py-2 rounded" onClick={checkVerifiedAndContinue} disabled={loading}>
                {loading ? <Spinner size={16} /> : 'I verified — continue'}
              </button>
            </div>
          </div>
        ) : (
          <button className="bg-blue-700 text-white py-2 px-4 rounded" onClick={handleSubmit} disabled={loading}>{loading ? <Spinner size={16} /> : 'Continue to Payment'}</button>
        )}
